...
//                System.out.print("PWD"+cursor.getString(cursor.getColumnIndex("PWD")));
    ...
    public void update(DBsqlite dop,int i){
        SQLiteDatabase db=dop.getWritableDatabase();
//        String sql="DELETE FROM DFinal WHERE 'Index' = "+i+" ;";
//        int dFinal = db.update("DFinal", "Index = " + "'"+i+ "'",null);
        db.close();
    }

    public void delete(DBsqlite dop,String  Keep){
        Thread t=new Thread(()->{
            SQLiteDatabase db=dop.getWritableDatabase();
            int dFinal = db.delete("DFinal", "Keep = '"+Keep+"'" ,null);
            db.close();
        });
        t.run();
